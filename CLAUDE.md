...
| organize Tailwind classes in JSX, extract repeated Tailwind class string to constant, use CVA for Tailwind variants, class-variance-authority Tailwind, replace child selector [&_a] with component prop, prettier-plugin-tailwindcss class order, normalize arbitrary Tailwind values to scale or theme tokens | `wip-tailwind-tailwind-classes-organization` |
